use crate::{
    ...
    sockets::{self, split_server_control_socket, WelcomeSocket},
    statistics::StatisticsManager,
    tracking::TrackingManager,
    AlvrButtonType_BUTTON_TYPE_BINARY, AlvrButtonType_BUTTON_TYPE_SCALAR, AlvrButtonValue,
    AlvrButtonValue__bindgen_ty_1, AlvrDeviceMotion, AlvrQuat, EyeFov, OculusHand,
    CLIENTS_UPDATED_NOTIFIER, HAPTICS_SENDER, IS_ALIVE, RESTART_NOTIFIER, SERVER_DATA_MANAGER,
...
use alvr_sockets::{
    spawn_cancelable, ClientListAction, ClientStatistics, StreamConfigPacket, StreamSocketBuilder,
    Tracking, AUDIO_STREAM, HAPTICS_STREAM, STATISTICS_STREAM, TRACKING_STREAM, VIDEO_STREAM,
};
...
use tokio::{sync::mpsc as tmpsc, time};
...
//     let (ip, handshake_packet) = sockets::search_client_loop(|handshake_packet| async move {
...
pub fn discovery_loop() -> IntResult {
    let welcome_socket = WelcomeSocket::new().map_err(to_int_e!())?;

    loop {
        check_interrupt!(IS_ALIVE.value());

        let (client_ip, client_hostname) = match welcome_socket.recv_non_blocking() {
            Ok(pair) => pair,
            Err(e) => {
                match e {
                    InterruptibleError::Interrupted => (),
                    InterruptibleError::Other(_) => thread::sleep(RETRY_CONNECT_MIN_INTERVAL),
                }
        ...
        let fdsf = split_server_control_socket(
            &[client_ip],
            Arc::clone(&IS_ALIVE),
        );

        visible_trusted_client_ips.lock().insert(client_ip);

        // let fhdjksfh = split_server_control_socket(client_ips, Arc<IS_ALIVE)
    }

    Ok(())
}

fn connect_to_manual_ips_loop() -> IntResult {
    loop {
        check_interrupt!(IS_ALIVE.value());

        // if let Some(fjds) = visible_trusted_client_ips.lock() {

        // }
        let manual_client_ips = SERVER_DATA_MANAGER
            .lock()
            .session()
            .client_connections
            .iter()
